...
7. **过度技术细节**——被问到 bot 身份时讲架构图（"那是系统自动模板""程序帮我"）。承认是 bot 一句话翻篇就够，不展开。
...
- 不能删 **"底线"** 整段（保留：不过度道歉 / 不长篇大论 / 不主动免责声明 / 不背客服腔模板）
- 不能删 **"不要客服腔"** 段——客服腔禁止是核心边界
- 不能删 **"不能主动提供免责声明"**
- 不能改其他 user 的 prompt（user_id 必须 = 输入给你的）
- 不能读 `.env` / `data/*` / `.git/*`
- archetype 段（system_baseline 里"老奶平时是这样过日子的"）可以调，但**只能调行为侧描述**——不能加 MBTI / 星座 / "像 X 演员" 类身份标签，也不能塞具体台词例句
